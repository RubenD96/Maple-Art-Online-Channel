...
public class FriendList {
    ...
    public void sendMessage(Packet packet) {
        friends.keySet().forEach(friendId -> {
            Character friend = Server.INSTANCE.getCharacter(friendId);
    ...
    public Packet getUpdateFriendsPacket() {
        ...
        for (Friend f : friends.values()) {
            boolean visible = f.isOnline() && Server.INSTANCE.getCharacter(f.getCharacterId()).getFriendList().getFriends().containsKey(owner.getId());
    ...
    public Packet getSendFriendRequestPacket(int cid) {
        Character friend = Server.INSTANCE.getCharacter(cid);
        if (friend != null) {
    ...
    public void notifyMutualFriends() {
        if (!owner.getClient().isCc()) {
            ...
            friends.keySet().forEach(f -> {
                Character friend = Server.INSTANCE.getCharacter(f);
                if (friend != null) {
